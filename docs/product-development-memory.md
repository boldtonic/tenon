...
- Product pill v1 added to the project canvas:
  - Collapsed product entry point with inline expansion, positioned as the bottom-left mini-pill on the canvas.
  ...
  - Nuxt DevTools is disabled by default to avoid a duplicate bottom-left pill; enable it explicitly with `NUXT_DEVTOOLS=true` when debugging Nuxt internals.
