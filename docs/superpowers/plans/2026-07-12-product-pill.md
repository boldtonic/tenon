...
**Goal:** Menú flotante de producto abajo al centro del lienzo 3D del editor, ocupando el hueco visual tipo DevTools que ya funcionaba bien: tema (claro/oscuro), idioma (ES/EN), reiniciar proyecto y volver al inicio, con modelo híbrido de expansión en línea.
...
Abrir un proyecto. Verificar: pill colapsado abajo-centro del lienzo, ocupando el hueco visual de la antigua cápsula de DevTools y sin duplicarse con ella; hover ~150 ms abre nivel 1 (tema + ES|EN + "···"); "···" pasa a nivel 2 (reiniciar + inicio, con etiquetas); mover el ratón fuera sin haber hecho clic → colapsa; clic en la T lo fija y entonces solo cierra con clic fuera o Escape.
